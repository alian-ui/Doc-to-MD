import { UnifiedCrawler, CrawlerAnalysis, UnifiedResult } from './unified-core';
import { DocToMdConfig, DEFAULT_CONFIG } from './config';

describe('UnifiedCrawler', () => {
  let mockConfig: DocToMdConfig;
  let crawler: UnifiedCrawler;
  ...
  beforeEach(() => {
    ...
    crawler = new UnifiedCrawler(mockConfig, 'https://example.com');
  });
  ...
  describe('analyzeWebsite', () => {
    ...
    test('should determine simple complexity for small sites', () => {
      const crawler = new UnifiedCrawler(mockConfig);
      // Access private method via any type for testing
      const complexity = (crawler as any).determineComplexity(5);
      expect(complexity).toBe('simple');
    });

    test('should determine moderate complexity for medium sites', () => {
      const crawler = new UnifiedCrawler(mockConfig);
      const complexity = (crawler as any).determineComplexity(25);
      expect(complexity).toBe('moderate');
    });

    test('should determine complex complexity for large sites', () => {
      const crawler = new UnifiedCrawler(mockConfig);
      const complexity = (crawler as any).determineComplexity(100);
    ...
    test('should detect proxy requirements', () => {
      ...
      const crawlerWithProxy = new UnifiedCrawler(proxyConfig);
      const needsProxy = (crawlerWithProxy as any).needsProxy();
    ...
    test('should detect formatting requirements', () => {
      ...
      const crawlerWithFormat = new UnifiedCrawler(formatConfig);
      const needsFormatting = (crawlerWithFormat as any).needsFormatting();
    ...
    test('should detect performance requirements', () => {
      ...
      const crawlerWithPerf = new UnifiedCrawler(perfConfig);
      const needsPerformance = (crawlerWithPerf as any).needsPerformance(150);
  ...
  describe('crawl', () => {
    test('should perform basic crawl for simple requirements', async () => {
      ...
      const simpleCrawler = new UnifiedCrawler(simpleConfig);
    ...
    test('should use analysis to select appropriate crawler', async () => {
      // Mock analysis
      const mockAnalysis: CrawlerAnalysis = {
        estimatedPages: 5,
        complexity: 'simple',
        requiresRetry: false,
        requiresProxy: false,
      ...
      (crawler as any).analysis = mockAnalysis;
  ...
  describe('crawler selection logic', () => {
    test('should select format crawler for formatting requirements', () => {
      ...
      const selectedCrawler = (crawler as any).selectOptimalCrawler(analysis);
    ...
    test('should calculate confidence correctly', () => {
      ...
      const confidence = (crawler as any).calculateConfidence(analysis);
