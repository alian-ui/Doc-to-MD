...
describe('Enhanced Formatter Test Suite', () => {
    ...
    describe('Formatter Initialization', () => {
        it('should initialize with default formatting options', () => {
            const formatter = new format_core_1.EnhancedFormatter(config);
            expect(formatter).toBeInstanceOf(format_core_1.EnhancedFormatter);
        });
        it('should initialize with custom formatting options', () => {
            ...
            const formatter = new format_core_1.EnhancedFormatter(config, customOptions);
            expect(formatter).toBeInstanceOf(format_core_1.EnhancedFormatter);
        });
    });
...
Some content here.`;
            const result = formatter.enhanceMarkdown(basicMarkdown);
...
Another paragraph.`;
            const result = formatter.cleanUpWhitespace(messyMarkdown);
...
- Item 3
More text`;
            const result = formatter.improveListFormatting(listMarkdown);
    ...
    describe('Table Formatting', () => {
        it('should format tables correctly', () => {
            ...
            const mockTable = {
                querySelectorAll: globals_1.jest.fn((selector) => {
                    if (selector === 'tr') {
                        return [
                            {
            ...
            const result = formatter.formatTable(mockTable);
        ...
        it('should handle empty tables', () => {
            ...
            const result = formatter.formatTable(mockEmptyTable);
            expect(result).toBe('');
        });
    });
    describe('Code Block Enhancement', () => {
        it('should detect programming languages correctly', () => {
            ...
            testCases.forEach(({ className, expected }) => {
                ...
                const result = formatter.detectLanguage(mockElement);
        ...
        it('should detect language from content patterns', () => {
            ...
            testCases.forEach(({ content, expected }) => {
                const mockElement = { className: '', textContent: content };
                const result = formatter.detectLanguage(mockElement);
                expect(result).toBe(expected);
            });
        });
...
Regular paragraph with [link](http://example.com) text.`;
            const wordCount = formatter.countWords(testMarkdown);
        ...
        it('should estimate reading time correctly', () => {
            ...
            const readingTime = formatter.estimateReadingTime(longText);
        ...
        it('should handle empty text', () => {
            ...
            expect(formatter.estimateReadingTime('')).toBe(0);
        });
    });
...
#### Sub-subsection`;
            const toc = formatter.generateTocEntries(markdownWithHeadings, 'http://example.com');
        ...
        it('should create proper anchors from titles', () => {
            ...
            testCases.forEach(({ title, expected }) => {
                const result = formatter.createAnchor(title);
                expect(result).toBe(expected);
            });
        });
        it('should nest table of contents correctly', () => {
            ...
            const nested = formatter.nestTableOfContents(flatToc);
    ...
    describe('Image and Link Formatting', () => {
        it('should format images with attributes', () => {
            const formatter = new format_core_1.EnhancedFormatter(config, formattingOptions);
            const mockImg = {
                getAttribute: globals_1.jest.fn((attr) => {
                    const attrs = {
                        src: 'image.jpg',
                        alt: 'Test Image',
                        title: 'Image Title',
            ...
            const result = formatter.formatImage(mockImg);
        ...
        it('should format links correctly', () => {
            ...
            testCases.forEach(({ content, href, title, expected }) => {
                ...
                const result = formatter.formatLink(content, mockLink);
                expect(result).toBe(expected);
            });
        });
    });
    describe('Metadata Generation', () => {
        it('should generate metadata header', () => {
            ...
            const header = formatter.generateMetadataHeader(mockResults);
        ...
        it('should generate statistics section', () => {
            ...
            const stats = formatter.generateStatsSection(mockResults);
...
Here are some examples.`;
            const enhanced = formatter.enhanceMarkdown(inputMarkdown);
            const wordCount = formatter.countWords(inputMarkdown);
            const readingTime = formatter.estimateReadingTime(inputMarkdown);
            const toc = formatter.generateTocEntries(inputMarkdown, 'http://example.com');
