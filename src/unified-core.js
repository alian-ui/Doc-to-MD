...
const site_optimizations_1 = require("./site-optimizations");
class UnifiedCrawler extends events_1.EventEmitter {
    ...
    async analyzeWebsite(url) {
        ...
        try {
            ...
            const siteOptimization = (0, site_optimizations_1.getOptimizedSelectors)(url);
            let navigationSelector = this.config.selectors.navigation;
            if (siteOptimization) {
                navigationSelector = siteOptimization.navigation;
                console.log(`🎯 Detected ${siteOptimization.siteName}: ${siteOptimization.notes}`);
                console.log(`🔧 Using optimized selectors`);
            }
            // Quick analysis using appropriate navigation selector
            const links = await (0, core_1.getNavigationLinks)(url, navigationSelector);
            const analysis = {
    ...
    enhanceConfidenceWithAdvancedAnalysis(baseConfidence, analysis) {
        let enhanced = baseConfidence;
        // Multi-factor confidence boost for 100% detection
        if (analysis.estimatedPages > 50)
            enhanced += 0.1; // Large documentation
        if (analysis.complexity === 'complex')
            enhanced += 0.1; // Complex structure detected
        if (analysis.estimatedPages > 100)
            enhanced += 0.1; // Very large site
